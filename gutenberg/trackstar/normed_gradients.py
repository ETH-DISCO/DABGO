...
def compute_normed_vectors_batched(gradient_files, autocorr_matrix, source="original"):
    ...
    gradient_dir = os.path.join(os.path.dirname(__file__), "../../data/trackstar/gutenberg/gradients")
    if source != "original":
        gradient_dir = os.path.join(os.path.dirname(__file__), f"../../data/trackstar/gutenberg/sample_gradients/{args.source}")
    for file_path in gradient_files:
        ...
        if source != "original":
            torch.save(gradients_list, os.path.join(os.path.dirname(__file__), f"../../data/trackstar/gutenberg/sample_gradients/{args.source}/normed_{file_path}"))
            print(f"Saved: {os.path.join(os.path.dirname(__file__), f"../../data/trackstar/gutenberg/sample_gradients/{args.source}/{file_path}")}")
            os.remove(os.path.join(os.path.dirname(__file__), f"../../data/trackstar/gutenberg/sample_gradients/{args.source}/{file_path}"))
            print(f"Deleted: {os.path.join(os.path.dirname(__file__), f"../../data/trackstar/gutenberg/sample_gradients/{args.source}/{file_path}")}")
        else:
...
if __name__ == "__main__":
    ...
    if args.source != "original":
        gradient_files = os.listdir(os.path.join(os.path.dirname(__file__), f"../../data/trackstar/gutenberg/sample_gradients/{args.source}"))
        gradient_files = [f for f in gradient_files if f.endswith(".pt")]
        print('gradient files', gradient_files)
    print('number of gradient files', len(gradient_files))
    autocorr_matrix = torch.load(os.path.join(os.path.dirname(__file__), '../../data/trackstar/gutenberg/autocorr_matrix_inv_sqrt.pt'), weights_only=False)
