...
def compute_blockwise_autocorrelation(gradient_files, eps=1e-12, device='cpu', start_index=0):
    ...
    os.makedirs(os.path.join(os.path.dirname(__file__), f"../../data/trackstar/gutenberg/autocorr_matrices"), exist_ok=True)
    ...
    for file_path in tqdm(gradient_files, total=len(gradient_files), desc="Accumulating autocorr"):
        ...
        if count % 100000 == 0:
            torch.save(sum_outer, os.path.join(os.path.dirname(__file__), f"../../data/trackstar/gutenberg/autocorr_matrices/autocorr_matrices_{count+start_index}.pt"))
            print('saved', count)
...
def compute_autocorr_matrices_inv_sqrt(auto_corr_files):
    ...
    with torch.no_grad():
        for auto_corr_file in auto_corr_files:
            auto_corr = torch.load(os.path.join(os.path.dirname(__file__), f"../../data/trackstar/gutenberg/autocorr_matrices/{auto_corr_file}"), map_location='cpu', weights_only=False)
            for block in blocks:
        ...
        torch.save(block_autocorr_inv_sqrt, os.path.join(os.path.dirname(__file__), f"../../data/trackstar/gutenberg/autocorr_matrix_inv_sqrt.pt"))
...
if __name__ == "__main__":
    ...
    auto_corr_files = os.listdir(os.path.join(os.path.dirname(__file__), f"../../data/trackstar/gutenberg/autocorr_matrices"))
