...
class TrackstarUnbatched:
    ...
    def compute_grouped_gradients_batch(self, sample, group_size=2):
        ...
        self.counter += 1
        if self.counter == self.number_of_samples-1 or self.counter % 15000 == 0:
            os.makedirs(os.path.join(os.path.dirname(__file__), '../data/trackstar/gutenberg/gradients'), exist_ok=True)
