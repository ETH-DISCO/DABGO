...
def make_loader(ds, batch_size=64):
    def collate(batch):
        ...
        ids = [torch.tensor(x, dtype=torch.long) for x in batch]
        pad_token_id = tokenizer.pad_token_id
        att = [torch.ones_like(torch.tensor(x), dtype=torch.long) for x in batch]
...
for batch in tqdm(loader):
    ...
    if idx % 100 == 0:
        ...
        np.save(os.path.join(os.path.dirname(__file__), "train_embeddings_gutenberg.npy"), all_embs)
