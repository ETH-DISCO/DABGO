...
def main(sample_name, source):
    ...
    corpus = torch.load(os.path.join(BASE_DIR, "data", "untokenized_wiki.pt"))
    print("Loaded docs:", len(corpus))
    tokenized_corpus = [doc.split(" ") for doc in corpus]
    bm25 = BM25Okapi(tokenized_corpus)
    file_dir = os.path.dirname(__file__)
    
    sample_names = os.listdir(os.path.join(BASE_DIR, "data", "samples_wikipedia"))
    sample_names = [name.replace(".pt", "") for name in sample_names]
    print(sample_names)
    for name in sample_names:
        sample = torch.load(os.path.join(BASE_DIR, "data", "samples_wikipedia", f"{name}.pt"), map_location='cpu', weights_only=False)
        ...
        os.makedirs(os.path.join(file_dir, "bm25", "sample_scores"), exist_ok=True)
        scores = np.array(scores)
        np.save(os.path.join(file_dir, "bm25", "sample_scores", f"{name}.npy"), scores)
